...
/*   Updated: 2023/01/29 16:01:12 by kyoulee          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_scn_camera	*ft_scn_camera_set(t_rt	*rt)
{
	...
	if (!ft_zeromalloc((void **)&camera_list->rt, sizeof(t_rt *) * (camera_list->max_index + 1)))
		ft_exit_print_error(ENOMEM, "ft_camera_set()");
	i = 0;
	while (rt[i].data)
	{
	...
	if (!ft_strcmp(camera_list->rt[camera_list->index]->type, "C"))
		camera_list->camera = (t_C *)(camera_list->rt[camera_list->index]->data);
	else
